        ...
        if(err)
          ...
          if(foundToken){
            User.findById(foundToken.user, function(err, foundUser){
              if(foundUser){
                res.status(200).json({id: foundUser._id, username: foundUser.username, name: foundUser.name, email: foundUser.email})
              }
            })
          }
          else{
            res.status(401).json({detail: "Acess denied. Invalid details!!!"})
          }
        }
      })
    }else{
